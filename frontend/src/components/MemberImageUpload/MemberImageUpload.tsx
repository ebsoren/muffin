...
export default function MemberImageUpload({ onImageUploaded, currentImage, className = '', ID}: ImageUploadProps) {
  ...
  const handleFileSelect = useCallback(async (file: File) => {
    ...
    setIsUploading(true);
    try {
      const result = await uploadImageToSupabase(file, "profile", ID?.toString() || null  );
  ...
  const currentImageUrl = currentImage ? getProfileImageUrl(currentImage, 96) : null;
        ...
        {isUploading ? (
          ...
          <div className="space-y-2">
            <div className="relative inline-block">
              <img
                src={previewUrl || currentImageUrl || '/default-avatar.svg'}
                alt="Preview"
